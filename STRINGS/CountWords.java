...
public class CountWords{
    public static int countWords(String str) {
        if (str.length() == 0) { // if input string is empty
            return 0;
        }
        int count = 1; // start with 1 because there's at least 1 word
        for (int i = 0; i < str.length(); i++) {
            if (str.charAt(i) == ' ') { // if current character is a space
                count++; // increment word count
            }
        }
        return count; // return final word count
    }
}